/**
 * Typed fixtures + route wiring for the Live Market page (`/live`).
 *
 * Endpoint fan-out, read off LiveMarketPage.tsx rather than off what the
 * spec happened to mock before:
 ...
 *   GET  /api/live/status                  useLiveStatus (via mockCommon)
 *
...
/** The backend also returns `source`/`stale_days`, which the frontend's
 *  ReferenceLevels doesn't model — declared in the satisfies target so the
 *  fixture stays faithful to the wire format without going unchecked. */
export const MOCK_REFERENCE_LEVELS = {
  ...
  source: 'mock',
  stale_days: 0,
} satisfies ReferenceLevels & { source: string; stale_days: number };
