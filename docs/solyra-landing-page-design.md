...
3. **Obsidian Analyst base + dawn accent.** Existing dark token system
   (`src/index.css`) with a new Solyra accent: amber→orange gradient
...
|---|---|
| Bento Gamma Map ladder | `src/components/options/ProfilesTab.tsx` D3 diverging bars (green +/purple −, red dashed spot, King/Gate badges) |
| Deep-dive gamma grid | `src/components/options/SwingMode.tsx` strike×expiry CSS grid (green/red cells, gold King cell, dashed spot & flip rows) |
| Chart overlay styles | `src/routes/ChartsPage.tsx:283-319` — King solid gold `#f59e0b` w2, Gate dotted blue `#3b82f6`, Flip dashed violet `#a78bfa` |
| Ladder alternative (if needed) | `src/components/options/TrinityTab.tsx` put/call ladders |
