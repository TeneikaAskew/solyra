...
| `/api` calls hit **staging** unexpectedly | `vite.config.ts` probes `localhost:8000` and falls back to `solyra-api-staging` when nothing answers. Start the backend, or set `VITE_API_PROXY_TARGET`. |
