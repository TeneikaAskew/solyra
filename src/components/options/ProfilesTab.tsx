...
// ── Profiles tab ───────────────────────────────────────────────────────────
// Verbatim move of the original OptionsFlowPage body — the toolbar
// (metric/filter/date-nav/spot-override), metrics bar, levels taxonomy chips,
// the D3 GEXHeatmap, error/loading/empty states and source footer. All real
// data wiring (useOptionsDates / useOptionsData / useOptionsGreeks /
// useGammaLevels) is preserved exactly.
interface ProfilesTabProps {
  activeTicker: Ticker;
}

export default function ProfilesTab({ activeTicker }: ProfilesTabProps) {
  ...
  // Reset the selected date index when the ticker changes. Uses React's
  // documented "adjusting state when a prop changes" pattern (store the
  // previous ticker in state, compare during render) — no setState-in-effect
  // and no ref access during render.
